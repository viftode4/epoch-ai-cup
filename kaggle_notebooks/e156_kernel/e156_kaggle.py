"""E156: Bearing fix retrain -- Kaggle GPU notebook."""
...
def _find_file(marker):
    """Search all of /kaggle/input/ for a file matching marker."""
    for p in INPUT.rglob(marker):
        return p
    raise FileNotFoundError(f"Cannot find {marker} under {INPUT}")

# Auto-discover paths regardless of Kaggle mount structure
_data_py = _find_file(os.path.join("src", "data.py"))
SRC_PARENT = _data_py.parent.parent  # parent of src/
_best_feat = _find_file("best_features.txt")
EXT_DATA_DIR = _best_feat.parent
# Find competition train.csv (not train_weather/train_solar)
COMP_DIR = None
...
# Also save OOF + test numpy for downstream
np.save("/kaggle/working/oof_e156.npy", best_oof)
np.save("/kaggle/working/test_e156.npy", best_test)
print("Saved oof_e156.npy, test_e156.npy", flush=True)
print("\nDone.", flush=True)
