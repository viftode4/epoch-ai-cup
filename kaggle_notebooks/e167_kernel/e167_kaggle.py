...
# If dataset was uploaded as zip, extract it first
zip_file = SRC_DIR / "epoch_src_bundle.zip"
if zip_file.exists() and not (SRC_DIR / "src" / "__init__.py").exists():
    print("Extracting epoch_src_bundle.zip...", flush=True)
    with zipfile.ZipFile(zip_file, "r") as zf:
        zf.extractall("/kaggle/working/epoch-src")
    SRC_DIR = Path("/kaggle/working/epoch-src")
    print(f"Extracted to {SRC_DIR}", flush=True)
elif not (SRC_DIR / "src" / "__init__.py").exists():
    # Try alternative: files might be directly in the dataset root
    # List what's available for debugging
    print(f"Contents of {SRC_DIR}:", flush=True)
