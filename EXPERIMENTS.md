...
| E107 | 2026-03-02 | decoupled_rankers_calibrated | **LB 0.42** (LOMO 0.4077) | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Decoupled Pairwise Rankers (LambdaMART) + Calibrated PP.** Mathematical insight: multi-class softmax couples probabilities, destroying intra-class ranking for minority classes (which macro-mAP evaluates). Trained 9 independent `XGBRanker(objective='rank:pairwise')` models to explicitly maximize Area Under PR Curve for each class. Calibrated the 9 unbounded ranking scores into a joint probability distribution using Logistic Regression on OOF predictions. Applied E96 (heading+ac1) unseen-month post-processing. LOMO jumped to **0.4077** (massive gain vs tree baseline ~0.36), but Kaggle scored **0.42**. This means calibrating decoupled rankers is highly brittle under covariate shift. |
| E108 | 2026-03-02 | wind_physics_base | **LB 0.49** (LOMO 0.2917) | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Wind-Compensated Intrinsic Base Model.** The 0.59 plateau is bottlenecked by the shared months (Sep/Oct). In autumn, Gulls/Waders/Pigeons are confused because raw `airspeed` (ground speed) is blurred by wind variance. E100 proved wind-compensated kinematics (v_air = v_ground - v_wind) provide orthogonal signal. By injecting `airspeed_wind`, `headwind`, and `crosswind` DIRECTLY into the base tree ensemble (E79 pruned features), the model can learn true flight effort and disentangle the classes in the shared months. LOMO dropped to 0.29, and Kaggle scored **0.49**. This proves that wind features are too noisy/unreliable to be used as primary splits in the tree ensemble, even if they work as a gentle post-processing prior (E100). |
| E109 | 2026-03-02 | shared_month_targeted_specialists | **LB 0.59** (LOMO 0.3769) | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Shared-Month Targeted Specialists (Gulls vs Waders vs Pigeons).** The 0.59 plateau is strictly bottlenecked by the shared months (Sep/Oct). Attempts to fix the base model globally (E107, E108) failed because they disrupt the delicate balance of the 0.59 base model. The remaining errors in Sep/Oct are highly localized: the model confuses Gulls, Waders, and Pigeons. Trained highly targeted binary specialists (Gulls vs Waders, Gulls vs Pigeons) using ONLY the shared months (Sep/Oct) data. Applied these specialists ONLY to samples in the shared months where the base model is uncertain between these specific pairs. This surgically fixes the bottleneck without touching the rest of the distribution. LOMO jumped from 0.3625 to 0.3769 (+0.0144). Kaggle scored **0.59**. This implies the shared-month errors are irreducible Bayes error given the current feature representation, or the public LB slice lacks enough shared-month minority samples to register the improvement. |
| E110 | 2026-03-02 | pseudo_label_distillation | **LB 0.11** | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Pseudo-Label Distillation (Student-Teacher).** Attempted to train a LightGBM student on soft pseudo-labels from the E101 teacher. Failed catastrophically (LB 0.11) due to a bug in the custom LightGBM multi-class objective function which scrambled the gradients across classes. |
| E111 | 2026-03-02 | mega_ensemble_geo5 | **LB 0.59** | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Mega Ensemble (Geometric Mean of 5x 0.59 Models).** To break the 0.59 plateau safely, we ensemble our 5 best, most diverse 0.59 submissions: E101 (Geo4), E109 (Targeted Specialists), E98 (Flock Evidence), E100 (Wind Evidence), and E75 (Altitude Evidence). This combines the unseen-month physics corrections with the shared-month specialist corrections, reducing variance and smoothing out edge-case errors. Kaggle scored **0.59**. The fact that ensembling 5 orthogonal models yields the exact same score implies we are missing a fundamental structural feature of the test set. |
| E113 | 2026-03-02 | graph_flock_smoothing | **LB 0.59** (LOMO +0.0128) | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Graph-Based Flock Smoothing (Exploiting the Test Set Time Leak).** Deep data analysis revealed that 74% of test tracks start within 60s of another track. The 0.61 team likely exploited this by grouping tracks from the same flock. However, heuristic time clustering fails because different species pass the radar simultaneously. We trained a Pairwise Link Prediction model (HistGradientBoosting) on the train set to predict `P(same_flock)` using purely *relative* features (Delta time, lon, lat, speed, altitude, size match), which are invariant to seasonal shift. We built a graph of test tracks with edges where `P > 0.5`, extracted connected components, and averaged the predictions of the E111 Mega Ensemble for each flock. On LOMO CV, this approach yielded a massive **+0.0128** mAP gain. Kaggle scored **0.59**. The public LB slice (24%) is completely saturated and unresponsive to massive internal CV gains. |
| E114 | 2026-03-03 | dann_sequence_mps | LB 0.44 | -- | -- | -- | -- | -- | -- | -- | -- | -- | **DANN sequence model (MPS).** Train a domain-adversarial 1D-CNN on `src.sequence.prepare_sequences()` (8 channels incl. RCS/alt derivatives) + tabular features (dropping ALL_TEMPORAL, dropping duration/track_length), with a gradient-reversal domain head (source=train vs target=test). Outputs: `e114_dann_raw_20260303_1151.csv`, `e114_dann_pp_heading_ac1_tau0.25_g0.10_priortau0.15_20260303_1151.csv`. |
| E115 | 2026-03-03 | mlls_month_prior_em | **DO NOT SUBMIT** (OOF -0.120) | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Month-wise MLLS/EM label-shift correction on posteriors (transductive).** Applied Saerens-style EM per month to adjust posteriors: \(q\\propto p\\odot(\\pi_m/\\pi_{train})^\\alpha\\), with \(\\pi_m\\) estimated from unlabeled month predictions. **Result:** collapses on train OOF (`oof_e50.npy`), producing degenerate priors and mAP drop from 0.3625 → 0.2426. This reproduces prior finding (E91/E92): BBSE/MLLS is unstable/misspecified under this competition’s shift (violates pure label-shift). Generated (likely-bad) candidates: `e115_mlls_em_geo5_alpha0.75_20260303_1158.csv`, `e115_mlls_em_geo5_alpha0.95_20260303_1158.csv`. |
| E116 | 2026-03-03 | soft_graph_propagation | **LB 0.59** | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Soft graph label propagation (fix E113 over-smoothing).** Train a same-flock edge model on train using `primary_observation_id` with richer relative features (Δt, haversine distance, Δalt_mid, Δalt_range, ΔRCS mean/std, Δheading_R, Δrcs_ac1, Δsize). Build test candidate edges within 120s/NN window, keep edges with \(P>0.62\), then iteratively update only low-margin nodes using confidence-weighted neighbor averaging (beta=0.55, 6 iters) while listening only to high-margin neighbors. Edge model AUC (train pair OOF): **0.993**. Output: `e116_soft_graph_prop_geo5_20260303_1201.csv`. Kaggle: **0.59** (no improvement). |
| E117 | 2026-03-03 | external_flightstyle_poe | LB TBD | -- | -- | -- | -- | -- | -- | -- | -- | -- | **External pretraining attempt (Col de la Croix 1988).** Train a LightGBM “flight-style teacher” on Col de la Croix wingbeat-pattern classes (wader continuous vs passerine bounding vs raptor vs flock vs other) using coarse physics features (altitude, ground speed, airspeed, climb rate). Apply to competition tracks (compute mean ground speed + climb from `trajectory`) and use teacher outputs as a gated PoE expert on top of E111 geo5. OOF (on `oof_e50.npy`) showed only **+0.0006** mAP at (tau=0.12, lam=0.35); class-style prototypes were nearly indistinguishable → weak transfer. Output: `e117_colflightstyle_geo5_tau0.12_lam0.35_20260303_1209.csv`. |
| E118 | 2026-03-03 | ssl_simclr_seq_embeddings | LB TBD (NEW) | SKF 0.4141 | 0.306 | 0.671 | 0.091 | 0.274 | 0.370 | 0.836 | 0.429 | 0.500 | 0.249 | **Trajectory signal via SSL, not supervised deep learning.** Self-supervised SimCLR pretraining on `src.sequence.prepare_sequences()` for ALL tracks (train+test), using trajectory-safe augmentations (jitter/scale/drop/time-mask). Freeze encoder, extract 96-dim embeddings, then train LightGBM on (simple tabular: airspeed/min_z/max_z/size one-hot + embeddings) with 5-fold SKF. OOF macro mAP **0.4141** (inflated vs LOMO; treat as proxy only). Output: `e118_ssl_simclr_lgbm_0.4141_20260303_1231.csv`; artifacts: `oof_e118.npy`, `test_e118.npy`. |
| E119 | 2026-03-03 | blend_e111_ssl | **LB 0.57** (w=0.10) | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Low-risk test: does SSL add orthogonal signal?** Geometric-mean blend of strong saturated baseline `E111 mega_ensemble_geo5` with the new SSL trajectory model `E118` at weights \(w\\in\\{0.10,0.20,0.30\\}\) for the E118 component. Kaggle: `e119_geo_blend_e111_e118_w0.10_20260303_1232.csv` scored **0.57** → SSL model is actively harmful when mixed globally. Do **not** submit w=0.20/0.30. |
| E120 | 2026-03-03 | gated_ssl_expert | **LB 0.58** (A) | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Gate SSL trajectory expert to unseen months + low-margin rows.** Start from E111 geo5 and apply a small geometric blend with E118 only on test rows in months {2,5,12} and only when base margin < tau. Candidates: A=`e120_gated_ssl_A_tau0.25_w0.12_20260303_1354.csv` (312 rows changed), B=`...tau0.30_w0.12...` (420 rows), C=`...tau0.25_w0.18...` (312 rows). Kaggle: **A scored 0.58** → SSL remains net harmful; if it has signal it’s too weak/noisy vs calibration mismatch. |
| E121 | 2026-03-04 | graph_sharp | **LB 0.59** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Graph-based sharpening on E111 geo5 base. Kaggle: `e121_graph_sharp_A_tau0.18_b0.10` scored **0.59**. |
| E122 | 2026-03-04 | router_shared | **LB 0.59** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Router for shared months with graph. Kaggle: `e122_router_shared_tau0.18_lam0.35_graph` scored **0.59**. |
| E123 | 2026-03-04 | router | **LB 0.59** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Router approach. Kaggle: `e123_router_A` scored **0.59**. |
| E124 | 2026-03-04 | graphcrf | **LB 0.59** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Graph CRF. Kaggle: `e124_graphcrf_A_lam0.90_b0.35_t6_d0.65` scored **0.59**. |
| E125 | 2026-03-04 | groupcon_poe | **LB 0.58** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Group consensus PoE. Kaggle: `e125_groupcon_poe_tau0.25_lam0.12` scored **0.58**. |
| E127 | 2026-03-05 | student_quota_pl | **LB 0.54** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Student model with quota-based pseudo-labels. Kaggle: `e127_student_quota_pl_r0.45_floor0.25` scored **0.54**. |
| E128 | 2026-03-05 | ssl_edges_graphcrf | **LB 0.59** | -- | -- | -- | -- | -- | -- | -- | -- | -- | SSL edge model + graph CRF. Kaggle: `e128_ssl_edges_union_graphcrf_l0.90_b0.35_t6_d0.65` scored **0.59**. |
| E129 | 2026-03-05 | localgroup_evidence_graphcrf | **LB 0.51** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Local group evidence + graph CRF. Kaggle: `e129_localgroup_evidence_graphcrf` scored **0.51**. |
| E130 | 2026-03-05 | highprec_edges_graphcrf | **LB 0.59** | -- | -- | -- | -- | -- | -- | -- | -- | -- | High-precision edges + graph CRF. Kaggle: `e130_highprec_edges_graphcrf_tau0.98` scored **0.59**. |
| E140 | 2026-03-05 | winterNB_maySpeed | **LB 0.58** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Winter NB + May speed prior. Kaggle: `e140_winterNB_maySpeed_a5_0.00_tau0.25_gw0.10_gm0.10_priortau0.15` scored **0.58**. |
| E141 | 2026-03-06 | disableMayPrior_winterNB | **LB 0.58** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Disable May prior + winter NB. Kaggle: scored **0.58**. |
| E143 | 2026-03-06 | external_kinematics_pretrain | **LB 0.40** | -- | -- | -- | -- | -- | -- | -- | -- | -- | External kinematics pretraining. Kaggle: scored **0.40**. Catastrophic failure. |
| E147 | 2026-03-06 | selective_kinrouter | **LB 0.57** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Selective kinematic router. Kaggle: A/B both scored **0.57**. |
| E148 | 2026-03-06 | pairmass_kinrouter | **LB 0.58** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Pair-mass kinematic router. Kaggle: `e148_pairmass_kinrouter_B_tau0.90_lam0.20_topk3` scored **0.58**. |
| E151 | 2026-03-07 | subset_pairmass | **LB 0.57** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Subset pair-mass. Kaggle: `e151_subset_pairmass_D_bm0.20` scored **0.57**. |
| E154 | 2026-03-07 | may_switch | **LB 0.58** | -- | -- | -- | -- | -- | -- | -- | -- | -- | May regime switching. Kaggle: `e154_may_switch_A_e140` scored **0.58**. |
| E155 | 2026-03-07 | may_regime | **LB 0.58** | -- | -- | -- | -- | -- | -- | -- | -- | -- | May regime models. Kaggle: D/E/A variants all scored **0.58**. |
| E156 | 2026-03-08 | bearing_fix_retrain | SKF 0.7718 | -- | -- | -- | -- | -- | -- | -- | -- | -- | Kaggle GPU notebook: retrain E79 with corrected bearing preprocessing. SKF 0.7718 (slightly below E79’s 0.7736). |
| E157 | 2026-03-08 | multirocket_lgb | SKF 0.6801 | -- | -- | -- | -- | -- | -- | -- | -- | -- | Kaggle GPU notebook: MultiRocket (2000 kernels) + LGB on v2 resampled sequences + 36 tabular features. Best variant B (combined): SKF 0.6801. MultiRocket alone: 0.5938. Sequences don’t help trees. |
| E158 | 2026-03-08 | 1dcnn_v2 | SKF 0.5605 | -- | -- | -- | -- | -- | -- | -- | -- | -- | Kaggle GPU notebook: 1D-CNN on v2 pad sequences (6ch). Pure CNN: 0.4089, CNN+tabular hybrid: 0.5605. Better than E06 CNN (0.5238) but still far below trees. |
| E160 | 2026-03-09 | optuna_feature_selection | **LB 0.54** | -- | -- | -- | -- | -- | -- | -- | -- | -- | Optuna feature+HP selection, LGB-only. SKF 0.7787 (best SKF). Kaggle: **0.54**. Overfits to training distribution. |
| E161 | 2026-03-09 | full_signal_features | SKF 0.7667 | 0.620 | 0.969 | 0.437 | 0.592 | 0.732 | 0.951 | 0.823 | 0.787 | 0.593 | All 13 feature extractors (188 feats). Feature dilution: worse than E79 (0.7736). But Cormorants +0.115 (0.31->0.44). |
| E162b | 2026-03-09 | physics_wx | **LB 0.54** | -- | -- | -- | -- | -- | -- | -- | -- | -- | 69 physics + 15 weather/solar = 84 feats. No temporal. SKF **0.7753** (best ever). Kaggle: **0.54**. More features = better SKF, worse LB. |
| E163 | 2026-03-09 | pseudo_label_augment | SKF 0.8484 (inflated) | -- | -- | -- | -- | -- | -- | -- | -- | -- | Pseudo-labeling + rare-class augmentation on 69 physics feats. PL hurts (-0.008). Augmentation inflates SKF via near-copy leakage. Orig-train mAP: A=0.6980, B(PL)=0.6904, C(Aug)=0.8456, D(both)=0.8484. |
| E163b | 2026-03-09 | pl_aug_36feat | **LB 0.49** | -- | -- | -- | -- | -- | -- | -- | -- | -- | E79’s 36 features + augmentation (target=200). Orig-train SKF 0.8678 (inflated). **Kaggle: 0.49.** Augmentation hurts LB. Simple trajectory transforms (rotation/speed/RCS noise) don’t generalize -- model memorizes near-copies. PL also tested: consistently -0.010. **Dead end: both pseudo-labeling and simple augmentation fail.** |
| E164 | 2026-03-09 | lomo_invariant | LOMO 0.3310 / SKF 0.6821 | 0.549 | 0.916 | 0.229 | 0.591 | 0.678 | 0.953 | 0.851 | 0.800 | 0.572 | **25 invariant features (no weather/solar).** Phase 0 analysis: all 7 wx + sol_daylight_hours are month proxies (MI ratio >2x). Removing them drops SKF from 0.7736 to 0.6821. Weather/solar carry real class signal despite being leaky — removing hurts. Variant B (27 feats, +2 borderline solar) not completed. **Not worth submitting.** |
| E165 | 2026-03-09 | test_calibration | **LB 0.51** | -- | -- | -- | -- | -- | -- | -- | -- | -- | **MLLS test-time label calibration on E79 predictions.** Per-month Bayes ratio adjustment. E_aggressive PP_then_MLLS (alpha: Feb=0.20, May=0.15, Dec=0.15, tau=0.35). IW-mAP predicted calLB=0.620, **actual LB=0.51**. MLLS estimates are wrong -- EM converges to bad proportions when base preds are biased. **IW-mAP validation is unreliable for MLLS-based PP.** Dead end. |
| E166 | 2026-03-20 | mega_external | SKF 0.7730 / LOMO 0.3630 | 0.611 | 0.965 | 0.351 | 0.769 | 0.800 | 0.972 | 0.897 | 0.873 | 0.720 | **76 external features (tidal, water, landuse, ERA5, CAPE, marine, soil, etc.) from 16 aligned datasets.** LOMO feature selection: adding ANY new features to E79's 36 hurts LOMO (0.362 -> 0.302-0.360). Top-N by importance also worse. E79 backward-eliminated set is optimal. Noise-aware sample weights from E79 OOF neutral. **Feature dilution confirmed at massive scale.** |
| E167 | 2026-03-20 | improved_pp_external | cal_LB 0.594 | -- | -- | -- | -- | -- | -- | -- | -- | -- | **PP with external data as NB evidence channels.** Tested 9 channels: grassland_dist (+0.0011 IW-mAP), true_airspeed (+0.0007), others neutral/negative. Combined channels dilute. Best: grassland_dist only, gamma=0.05, tau=0.20. Improved GBIF priors (corvid-separated) actually hurt. Cal_LB 0.594 vs raw E79 0.600 — PP still slightly hurts this base. |
| E168 | 2026-03-20 | stacking_external | SKF 0.7273 | 0.540 | 0.949 | 0.207 | 0.669 | 0.815 | 0.963 | 0.889 | 0.865 | 0.649 | **Meta-learner (LGB) on E79 OOF + 14 external features.** LOMO 0.5811 (inflated — uses base preds as input). SKF 0.7273 worse than E79 (0.7736). Best blend alpha=0.00 (pure E79). **Stacking with external data doesn't help.** |
| E169 | 2026-03-20 | domain_informed | LOMO 0.3557 | -- | -- | -- | -- | -- | -- | -- | -- | -- | **29 domain-engineered features** (insect_drift, cormorant_residual, wader_tidal, duck_rain, geese_grassland, bop_thermal, Alerstam likelihoods, RCS scintillation, altitude censoring). dom_track_heading #1 importance (2461), dom_wader_tidal #2 (2003). ALL 6 feature combinations worse than E79 on LOMO. Feature replacement also worse. |
| E169b | 2026-03-20 | movebank_pp | cal_LB 0.597 | -- | -- | -- | -- | -- | -- | -- | -- | -- | **40.8M Movebank GPS events -> per-class speed priors.** Movebank SDs too broad (6-10 m/s, includes non-flight). Training data priors beat Movebank AND Alerstam. Domain channels (cormorant wind model, grassland, tidal) all neutral. Best: standard train priors, g=0.05, t=0.20. |
| E170 | 2026-03-20 | feature_rebuild_v2 | SKF 0.7013 | 0.639 | 0.903 | 0.296 | 0.725 | 0.631 | 0.966 | 0.876 | 0.838 | 0.438 | **Complete feature system rebuild (v3 after audit).** 76 features from features_v2.py: dual-scale RCS, track_heading (#1), grassland_dist (#6), headwind (fixed convention), moon_illumination (#12), crepuscular_index, pressure_trends, tidal, BLH. Fixed: headwind formula (was outputting ~0), LGB/CB double-weighting, XGB early stopping. StratifiedGroupKFold. LOMO 0.5141 (+0.151 vs E79). Ducks +0.059 (headwind+crepuscular), Pigeons +0.622, Clutter +0.293. Spatiotemporal smoothing +0.005. |
