...
| E81 | 2026-02-25 | nbalt_shared_months | LB TBD | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Test whether physics evidence helps on shared months too.** Keep best unseen-month correction (priors tau=0.15 + NB-alt on unseen months with `tau_nb=0.25, gamma=0.10, w_alt_range=0.50`, 451 rows), then additionally apply a *small, strictly-gated* NB-alt update on shared months (9/10). Two variants: (A) shared `tau=0.10, gamma=0.08` (322 rows) saved as `e81_nbalt_shared_tau0.10_g0.08_unseenTau0.25_unseenG0.10_wr0.50_20260225_1139.csv`; (B) shared `tau=0.15, gamma=0.06` (466 rows) saved as `e81_nbalt_shared_tau0.15_g0.06_unseenTau0.25_unseenG0.10_wr0.50_20260225_1139.csv`. |
...
| E87 | 2026-02-25 | validated_pp | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Apply LOMO-validated PP to E79 base.** Best configs: v1(g=0.50,t=0.30) 38 flips, v2(g=0.30,t=0.30) 30 flips, v5(g=0.10 E75-config) 21 flips. Also tested boosted Feb GBIF (1.5x, 2.0x). 9 submission variants for LB validation. |
...
| E92 | 2026-02-26 | full_pipeline | -- | -- | -- | -- | -- | -- | -- | -- | -- | -- | **Full pipeline: label shift + GBIF + NB physics.** 28 configs across 8 strategies (A-H). A: E87 ref (21-38 flips). B: E79+MLLS79 (57-99 flips). C: MLLS+GBIF (57-58 flips). D: BBSE (46-70 flips). E: E90+MLLS90 (44-110 flips). F: hybrid E79+MLLS90 (57-103 flips). G: hybrid+GBIF (58-59 flips). H: all-month (143 flips, 86 shared). 14 priority submissions saved. LB TBD. |
...
### Product-of-experts post-processing (E73–E78, LB ≥ 0.58)
