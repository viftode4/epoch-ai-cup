"""E74: Tune E73 Naive-Bayes physics correction (gate/strength).

E73 achieved LB=0.58 using a simple within-unseen-month correction:
  q_{i,c} ∝ p_{i,c} · L_{i,c}^gamma
where L_{i,c} is a Naive Bayes likelihood from (radar_bird_size, airspeed).

This script keeps the same pipeline (E50 -> E67 priors -> NB correction) and
generates a couple of nearby variants by adjusting:
  - tau_nb: uncertainty gate for applying the NB correction
  - gamma: strength of the product-of-experts factor
"""
...
# NB stage candidates: (tau_nb, gamma)
CANDIDATES: list[tuple[float, float]] = [
    (0.30, 0.14),  # broader gate + slightly stronger than E73
    (0.20, 0.10),  # more selective gate
]
...
def build_nb_params(train_df: pd.DataFrame) -> tuple[list[str], np.ndarray, np.ndarray, np.ndarray]:
...
def compute_nb_factors(
    ...
    speed = pd.to_numeric(test_df["airspeed"], errors="coerce").values.astype(float)
    ok = np.isfinite(speed)

    loglik = log_p_size[:, size_idx].T  # (N,K)
    if ok.any():
        loglik[ok] = loglik[ok] + log_gaussian(speed[ok], mu_speed, sig_speed)
...
print("=" * 70, flush=True)
print("E74 NB-PHYS TUNING (E73 FOLLOW-UP)".center(70), flush=True)
print("=" * 70, flush=True)
...
# NB factors
size_levels, log_p_size, mu_speed, sig_speed = build_nb_params(train_df)
factors, speed_ok = compute_nb_factors(test_df, size_levels, log_p_size, mu_speed, sig_speed)
...
for tau_nb, gamma in CANDIDATES:
    gate = unseen_mask & speed_ok & (margin0 < tau_nb)
    ...
    save_submission(
        out,
        f"e74_nbphys_unseen_tau{tau_nb:.2f}_g{gamma:.2f}_priortau{TAU_PRIOR:.2f}",
        cv_map=None,
    )
