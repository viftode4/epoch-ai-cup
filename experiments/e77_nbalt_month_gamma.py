"""E77: Month-specific gamma for the successful NB-alt correction.

Status so far
-------------
- E67 (GBIF ratio priors, gated) reached LB 0.56.
- E73/E74 added per-sample NB physics (size+speed) -> LB 0.58.
- E75 added altitude (alt_mid, alt_range) -> LB 0.59 (best).
- E76 added track-length cues -> LB 0.58 (hurt; likely non-invariant).

Hypothesis
----------
E75 applies the NB-alt correction uniformly across all unseen months (2/5/12).
But the *magnitude* of the remaining shift may differ by month:
  - Feb/Dec are winter-like (largest behavioral differences),
  - May is spring-like (closer to train month 4).

We keep the same NB-alt factors and gate, but apply a *month-specific* tempering:
  q_{i,c} ∝ p_{i,c} · factors_{i,c}^{gamma_{m_i}}

This is a targeted, low-parameter way to reduce over-correction risk in May
while preserving winter corrections.
"""
...
# NB-alt stage (month-specific gammas)
TAU_NB = 0.30
GAMMA_BY_MONTH = {2: 0.10, 5: 0.06, 12: 0.10}
...
print("=" * 70, flush=True)
print("E77 NB-ALT MONTH-GAMMA (E67 + NB-alt)".center(70), flush=True)
print("=" * 70, flush=True)
...
# NB-alt factors (same across months)
size_levels, log_p_size, mu, sig = build_nb_params(train_df)
factors = compute_nb_factors(test_df, size_levels, log_p_size, mu, sig)

margin0 = top2_margin(pred0)
gate_common = (margin0 < TAU_NB) & np.isin(test_months, UNSEEN_MONTHS)

out = pred0.copy()
print("\nNB-alt stage (month-specific gamma):", flush=True)
for m in UNSEEN_MONTHS:
    gamma = float(GAMMA_BY_MONTH.get(m, 0.0))
    mask = gate_common & (test_months == m)
    out[mask] = out[mask] * (factors[mask] ** gamma)
    out[mask] = renorm_rows(out[mask])
    print(f"  month={m}: gamma={gamma:.3f} gated_rows={int(mask.sum())}", flush=True)

out = renorm_rows(out)
save_submission(
    out,
    f"e77_nbalt_monthgamma_m2_{GAMMA_BY_MONTH[2]:.2f}_m5_{GAMMA_BY_MONTH[5]:.2f}_m12_{GAMMA_BY_MONTH[12]:.2f}_tau{TAU_NB:.2f}",
    cv_map=None,
)
