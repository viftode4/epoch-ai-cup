"""E81: Apply NB-alt evidence update on shared months too (Sep/Oct).

So far, the best family (E75/E78/E79) applies the physics likelihood update
only on unseen months (2/5/12). That was the minimal-scientist choice because
the original shift problem is unseen months.

However, once we have a physics-based evidence model that appears stable
(size+speed+alt_mid+alt_range), it may also help fix *within-month* confusions
on the shared months (9/10), which are the majority of the test set.

This experiment keeps the best-known unseen-month correction and adds a small,
strictly-gated correction on months 9/10.

Pipeline
--------
1) Base: test_e50.npy
2) Apply E67 gated GBIF ratio priors on unseen months (tau_prior=0.15)
3) Apply NB-alt evidence update:
   - Unseen months (2/5/12): tau_nb_unseen=0.25, gamma_unseen=0.10
   - Shared months (9/10): tau_nb_shared in {0.10, 0.15}, gamma_shared smaller
"""
...
# Evidence model (fixed; based on E78/E79)
GAMMA_UNSEEN = 0.10
TAU_NB_UNSEEN = 0.25
W_ALT_RANGE = 0.50

# Shared-month add-on variants
...
print("=" * 70, flush=True)
print("E81 NB-ALT ON SHARED MONTHS (E75/E78 FOLLOW-UP)".center(70), flush=True)
print("=" * 70, flush=True)
...
mask_unseen = np.isin(test_months, UNSEEN_MONTHS) & (margin0 < TAU_NB_UNSEEN)
print(f"Unseen NB: tau={TAU_NB_UNSEEN:.2f} gamma={GAMMA_UNSEEN:.2f} rows={int(mask_unseen.sum())}", flush=True)

base_out = pred0.copy()
base_out[mask_unseen] = base_out[mask_unseen] * (factors[mask_unseen] ** GAMMA_UNSEEN)
base_out = renorm_rows(base_out)

print("\nGenerating shared-month variants...", flush=True)
for tau_shared, gamma_shared in VARIANTS:
    out = base_out.copy()
    # recompute margin after unseen update (shared months unchanged by unseen update)
    order = np.argsort(-out, axis=1)
    p1 = out[np.arange(out.shape[0]), order[:, 0]]
    p2 = out[np.arange(out.shape[0]), order[:, 1]]
    margin_shared = p1 - p2

    mask_shared = np.isin(test_months, SHARED_MONTHS) & (margin_shared < tau_shared)
    out[mask_shared] = out[mask_shared] * (factors[mask_shared] ** gamma_shared)
    out = renorm_rows(out)

    print(
        f"  shared: tau={tau_shared:.2f} gamma={gamma_shared:.2f} rows={int(mask_shared.sum())}",
        flush=True,
    )
    save_submission(
        out,
        f"e81_nbalt_shared_tau{tau_shared:.2f}_g{gamma_shared:.2f}_unseenTau{TAU_NB_UNSEEN:.2f}_unseenG{GAMMA_UNSEEN:.2f}_wr{W_ALT_RANGE:.2f}",
        cv_map=None,
    )
