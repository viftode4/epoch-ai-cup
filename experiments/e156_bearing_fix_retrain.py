"""E156: Bearing fix retrain.

Retrain E79 pipeline (36-feature tree ensemble) to measure the impact of the
bearing distortion fix in src/features.py. The bearing_change_mean feature was
distorted by ~40% (using raw lat/lon degree diffs instead of meter-scaled diffs).

Compare SKF mAP vs E79's 0.7736, especially Cormorants, BoP, Waders.
"""
...
# ====================================================================
print("=" * 70, flush=True)
print("E156 BEARING FIX RETRAIN (SKF)".center(70), flush=True)
print("=" * 70, flush=True)
...
print_results(base_map, base_per, label="E156 ensemble (SKF OOF)")
...
print_results(best_map, best_per, label="E156 final (SKF OOF)")

# -- Save artifacts ------------------------------------------------
print("\nSaving artifacts...", flush=True)
np.save(ROOT / "oof_e156.npy", best_oof)
np.save(ROOT / "test_e156.npy", best_test)
save_submission(best_test, "e156_bearing_fix_retrain", cv_map=best_map)
